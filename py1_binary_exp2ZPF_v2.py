...
import glob
###################################################################################
first_elem ='CR'          # can be upper or lower letters for elements, will adjust them to upper later

weightTL   = 1            # Weight of tie lines
weightINV  = 50           # Weight of Inv equilibria
nTL_step   = 3            # for tie lines, the step: 1 for all; 2 half TL, 3 for 1/3 TL, ...
...
def find_two_elems(textp):
    ...
    for line in aa_lines:
        ...
        if " YTEXT " in line:
            filtered_line = [val for val in line.split(' ') if val != '']
            elem2=filtered_line[1]   # for binary case only, is not useful
...
####################################################################################
#---------------------
#my_file=open("correctedPD_CrHf.exp",'r')     # open Thermo-Calc EXP file: binary system
...
with open('temp1.txt', 'w') as f:
    f.write(str(2))  # 3 for ternary and 2 for binary
    f.close()
### ---------------------
...
print()
print('========================================================')
print('Updated on 2019-07-19: read EXP first one, and second_elem auto')
